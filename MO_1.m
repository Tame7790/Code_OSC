function [B8,POWER_1,POWER_2] = MO(targetES ,A0,B,TASK,number_123456,REWARD)
%A8:COST B8:VOLUME
Atarget=A0{targetES};
reward=REWARD{targetES};
...
while p2<=TASKnumber
    ...
    if TASKCPUcycle(p2)/MAXfrequency<=TASKdeadline(p2)
        ...
        if Plocal<=Btran(targetES+1)
            B8(p2,1+TASK+1+targetES)=Plocal*2*Btran(targetES+1);
    ...
    if p_tran(p2)<=MAXtranpower&&p_tran(p2)*(TASKdeadline(p2)-Tbackhaul)<=Btran(targetES+1)&&TASKdeadline(p2)>Tbackhaul
        ...
        B8(p2,1+TASK+1)=2*Btran(targetES+1)*POWER_1(p2,1+TASK+1)+V*price2*TASKbit(p2);%%%%clould prize:
    else
        B8(p2,1+TASK+1)=inf;
            ...
            if distance_ES==inf
                ...
                if sqrtm(Btran(p3+1) /(k*TASKCPUcycle(p2)))<=TASKCPUcycle(p2)/TASKdeadline(p2)||MAX1frequency<=TASKCPUcycle(p2)/TASKdeadline(p2)
                    ...
                    if a>MAXtranpower||a*TASKdeadline(p2)>Btran(targetES+1)
                        ...
                        B8(p2,1+TASK+1+p3)=2*Btran(targetES+1)*power1(min_x,min_y)+2*Btran(p3+1)*power2(min_x,min_y);
